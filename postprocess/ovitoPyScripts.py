...
if verbose:
	print('args:',kwargs)
#--- command-line args
InputFile = kwargs['InputFile'] #sys.argv[1] #--- input lammps file 
OutputFile = kwargs['OutputFile'] #sys.argv[2] #--- output
nevery = int(kwargs['nevery']) #int(sys.argv[3]) #--- process. frequency
AnalysisType = int(kwargs['AnalysisType']) #int(sys.argv[4]) #--- 0:CommonNeighborAnalysis, 1:g(r), 2:d2min, 3:voronoi analysis, 4 & 6: neighbor list, 5: dislocation analysis, 7: convert to dump, 8: displacements, 9: Periodic Image 10: nearest neighbor finder 11: Wigner-Seitz algorithm 
#print('AnalysisType=',AnalysisType)
 
if 'RefFile' in kwargs: #AnalysisType == 11: 
    RefFile = kwargs['RefFile'] #sys.argv[5]
if AnalysisType == 8: 
#    RefFile = sys.argv[5]
    use_frame_offset = False
    try:
        use_frame_offset = eval(kwargs['use_frame_offset']) #eval(sys.argv[6])
    except:
        pass
    if use_frame_offset:
       start_frame = 1
if 'cutoff' in kwargs:# AnalysisType == 4 or AnalysisType == 6: #--- neighbor lists
    cutoff = float(kwargs['cutoff']) #float(sys.argv[5])
#    if AnalysisType == 4:
#        natoms = int(sys.argv[6])
if 'OutputFile_headers' in kwargs: #AnalysisType == 7:
    OutputFile_headers = kwargs['OutputFile_headers'] #sys.argv[5] 
if 'pbc_false' in kwargs:
    pbc_false = int(kwargs['pbc_false']) #int(sys.argv[5]) 
...
if AnalysisType == 0:
    io.export_file( pipeline, OutputFile, "lammps_dump",\
                    columns = ["Particle Identifier", "Particle Type", "Position.X","Position.Y","Position.Z",\
                               "Structure Type"],
                     start_frame = 0,
                     end_frame = frame,#pipeline.source.num_frames,
...
if AnalysisType == 9:
    io.export_file( pipeline, OutputFile, "lammps_dump",\
                    columns = ["Particle Identifier", "Particle Type", "Position.X","Position.Y","Position.Z"],
                     start_frame = 0,
                     end_frame = frame,#pipeline.source.num_frames-1,
                     every_nth_frame = nevery,
                    multiple_frames=True 
                  )   
if AnalysisType == 7: 
